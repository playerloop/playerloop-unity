    ...
    public class PlayerLoopClient : MonoBehaviour
    {
        ...
        private IEnumerator UploadReport(PlayerLoopReport @event)
        {
            ...
            else
            {
                ...
                if (@event.localAttachmentPaths != null && @event.localAttachmentPaths.Count > 0 && @event.localAttachmentPaths[0] != null && @event.localAttachmentPaths[0] != "")
                {
        ...
        public void SendReport(string ReportMessage, bool userPrivacyAccepted, List<string> attachmentsFilePaths = null, string OptionalUserIdentifier = null)
        {
            ...
            {
                Debug.LogError("No secret token was set up! Add your secret to this object. Check https://playerloop.io/unity-tutorial for more information");
                reportErrorInSending.Invoke();
                return;
            }
            ...
            { //if the user did not accept the privacy policy, do not transmit PII
